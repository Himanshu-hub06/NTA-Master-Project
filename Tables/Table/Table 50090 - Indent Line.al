table 50090 "Indent Line"
{
    ...
    fields
    {
        ...
        {
            DataClassification = ToBeClassified;
            TableRelation = IF (Type = CONST(Item)) Item."No." WHERE("Item Category Code" = FIELD("Item Category"),
                                                                  Blocked = CONST(false));
            trigger OnValidate()
            begin
                TestStatusOpen();
                ...
                IF IndentHeader.FIND('-') THEN BEGIN
                    IF IndentHeader."Location Code" <> '' THEN BEGIN
                        ...
                        Priority := IndentHeader.Priority;
                    END ELSE
                        ERROR('Indent location should not be blank in Indent Header');
                END;
                IndentLine.RESET;
                IndentLine.SETRANGE(IndentLine."Document No.", "Document No.");
                IndentLine.SETRANGE(IndentLine."No.", "No.");
                IndentLine.SETRANGE(IndentLine.Type, IndentLine.Type::Item);
                IF IndentLine.FINDFIRST THEN BEGIN
                    ERROR('Item already exist in this document');
                END;
            end;
        }
        ...
        field(10; "Direct Unit Cost"; Decimal)
        {
            ...
            begin
                //TestStatusOpen();
                "Estimated Cost" := "Direct Unit Cost" * Quantity;
                IF "GST %" <> 0 THEN
        ...
        field(50002; "Estimated Cost"; Decimal)
        {
            ...
            begin
                //TestStatusOpen();
                "Direct Unit Cost" := "Estimated Cost" / Quantity;
            end;
        }
        ...
        field(60012; "Select Item For Proposal"; Boolean)
        {
            DataClassification = ToBeClassified;

            trigger OnValidate()
            begin
                IF "Purchase Proposal Created" THEN
                    ERROR('This indent Line allready "Purchase Praposal Created"');
            end;
        }
        field(60013; "Purchase Proposal Created"; Boolean)
        {
            DataClassification = ToBeClassified;
        }
        field(60014; "Actual Requisition Qty"; Decimal)
        {
            DataClassification = ToBeClassified;
            DecimalPlaces = 0 : 5;
        }
        field(60016; "Purchase Proposal No."; Code[20])
        {
            DataClassification = ToBeClassified;
        }
        field(60017; "Purchase Proposal Line No."; Integer)
        {
            DataClassification = ToBeClassified;
        }
    ...
    begin
        IF IndentHeader.GET("Document No.") THEN
            IndenHeader.TESTFIELD(IndenHeader.Status, IndenHeader.Status::Open);
        //IF IndentHeader.Posted THEN
    ...
    var
        ...
        Text003: Label 'Status must be Released for Indent %1';
        ...
        mitemline: Record 32;
        mremqty: Decimal;
        mrequi1, ReqLine : Record 50088;
        // ReqLine: Record 50171;
        mreqno: Code[20];
        mreqlineno: Integer;
        IndenHeader: Record 50089;
 ...
 IF "Line No." <> 0 THEN
   DimMgt.UpdateDocDefaultDim(
    ...
    begin
        ...
        IndenHeader.RESET;
        IndenHeader.SETRANGE(IndenHeader."No.", "Document No.");
        IF IndenHeader.FINDFIRST THEN BEGIN
            "Shortcut Dimension 1 Code" := IndenHeader."Shortcut Dimension 1 Code";
            "Shortcut Dimension 2 Code" := IndenHeader."Shortcut Dimension 2 Code";
            "Dimension Set ID" := IndenHeader."Dimension Set ID";
            "Location Code" := IndenHeader."Location Code";
            "Indent Date" := IndenHeader."Indent Date";
            "Consumption Type" := IndenHeader."Consumption Type";
