page 50188 "Admin Invoices Accepted"
{
    AdditionalSearchTerms = 'vendor invoice';
    ApplicationArea = Basic, Suite;
    Caption = 'Admin Invoices Accepted';
    CardPageID = "Admin Invoice";
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                field("Loc Fund Entry No."; rec."Loc Fund Entry No.")
                {
                    ApplicationArea = All;

                }
                field("Account Accepted Date"; Rec."Account Accepted Date")
                {
                    ApplicationArea = All;
        ...
        area(processing)
        {
            group(Invoice)
            {
                ...
                action(Approvals)
                {
                    ...
                    begin
                        // WorkflowsEntriesBuffer.RunWorkflowEntriesPage(Rec.RecordId, DATABASE::"Purchase Header", "Document Type".AsInteger(), "No.");
                    end;
                }
                action(Vendor)
                {
            ...
            group("P&osting")
            {
                ...
                action("Release Funds")
                {
                    ...
                    begin
                        ...
                        if PurchTab.Findfirst then
                            repeat
                                if PurchTab."Loc Fund Entry No." <> '' then PurchTab."Loc Fund Entry No." := '';
                                PurchTab.Modify();
