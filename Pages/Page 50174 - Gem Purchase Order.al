page 50174 "GeM Purchase Order"
{
    Caption = 'GeM Purchase Order';
    PageType = Document;
    ...
    SourceTableView = WHERE("Document Type" = FILTER(Order), "Admin Bill" = Const(True));
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Admin Bill"; Rec."Admin Bill")
                {
                    ApplicationArea = All;
            ...
            part(PurchLines; "GeM Purchase Order Subform")
            {
                ApplicationArea = Suite;
                Editable = Rec."Buy-from Vendor No." <> '';
    ...
    actions
    {
        ...
        area(processing)
        {
            ...
            group(FillPO)
            {
                Caption = 'Fill PO From Proposal';
                action(FillPOFromProposal)
                {
                    ApplicationArea = All;
                    caption = 'Fill PO From Proposal';
                    Promoted = true;
                    ...
                    trigger OnAction()
                    begin
                        CurrPage.PurchLines.Page.GetPurchReqLines();
                    end;
                }
            }
            group(Action13)
    ...
    //begin
    //    "Purchase Type" := "Purchase Type"::GeM;
    //end;
