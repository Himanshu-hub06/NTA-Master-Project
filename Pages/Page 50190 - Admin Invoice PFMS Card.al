page 50190 "Admin Invoice After PFMS"
{
    Caption = 'Admin Invoice After PFMS';
    PageType = Document;
    ...
    layout
    {
        area(content)
        {
            ...
            part(PurchLines; "Admin Invoice PFMS Subform")
            {
                ApplicationArea = Basic, Suite;
                Editable = Rec."Buy-from Vendor No." <> '';
        ...
        area(processing)
        {
            ...
            group("F&unctions")
            {
                ...
                action("Get Sanction Status")
                {
                    ...
                    begin
                        PFMSAdminCu.CreateXMLSanctionRequest(Rec."Sanction No.");
                        UrlText := 'http://20.219.125.221/get_sanction.aspx/?sno=' + Rec."Sanction No.";
                        Client.Get(UrlText, Response);
                        if Response.IsSuccessStatusCode then
                            PFMSCu.GetSanctionStatus();
                    end;
                }
                action("Get Payment Status")
                {
                    ...
                    var
                        ...
                        Content: HttpContent;
                    begin
                        PFMSAdminCu.CreateXMLPaymentDetailRequest(Rec."Sanction No.");
                        UrlText := 'http://20.219.125.221/get_payment_detail.aspx/?sno=' + Rec."Sanction No.";
                        //Client.Post(UrlText, Content, Response);
                        Client.Get(UrlText, Response);
                        if Response.IsSuccessStatusCode then
                            PFMSCu.GetPaymentStatus(Rec."Sanction No.");
                    end;
                }
                action(CopyDocument)
                {
