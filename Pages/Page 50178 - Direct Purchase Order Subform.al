page 50178 "Direct Purchase Order Subform"
{
    AutoSplitKey = true;
    Caption = 'Lines';
    DelayedInsert = true;
    ...
    layout
    {
        area(content)
        {
            repeater(Control1)
            {
                ...
                {
                    ApplicationArea = Suite;
                    Enabled = (Rec.Type <> Type::"Fixed Asset") AND (Rec.Type <> Type::" ");
        ...
        area(processing)
        {
            action(FillPO)
            {
                ...
                var
                    ...
                    ProLinesPoFom: Page "GeM Proposal Po Fill Line List";
                    ProposalLine: Record "Purchase Proposal Line";
                begin
                    ProposalLine.RESET;
                    ProposalLine.SETRANGE(ProposalLine."Location Code", PurchLine."Location Code");
                    ProposalLine.SETRANGE(ProposalLine."Purchase Type", ProposalLine."Purchase Type"::Direct);
                    ProposalLine.SETRANGE(ProposalLine."PO Selected", FALSE);
                    ProLinesPoFom.SETTABLEVIEW(ProposalLine);
                    ProLinesPoFom.GetDocument(PurchLine."Document No.");
                    ProLinesPoFom.RUN;
            ...
            group("&Line")
            {
                ...
                action(ItemChargeAssignment)
                {
                    ...
                    Image = ItemCosts;
                    Enabled = Rec.Type = Type::"Charge (Item)";
    ...
    var
        "ProposalLine.": Record "Purchase Proposal Line";
        proposalline: Record "Purchase Proposal Line";
        ProposalLinesPoFom: Page "GeM Proposal Po Fill Line List";
        PurchLineRec: Record "Purchase Line";
    begin
        ProposalLine.RESET;
        // ProposalLine.SETRANGE(ProposalLine."Location Code", Loc);
        ProposalLine.SETRANGE(ProposalLine."Purchase Type", ProposalLine."Purchase Type"::Direct);
        ProposalLine.SETRANGE(ProposalLine."PO Selected", FALSE);
        ProposalLinesPoFom.SETTABLEVIEW(ProposalLine);
        ProposalLinesPoFom.GetDocument(Rec."Document No.");
        ProposalLinesPoFom.RUN;
    end;
